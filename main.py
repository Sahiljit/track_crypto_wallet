                ...
                for tx in transactions:
                    ...
                    if tx_hash not in latest_tx_hashes and tx_time > last_run_time:
                        if tx["to"].lower() == wallet_address.lower():
                            ...
                            usd_value = value * (
                                eth_usd_price if blockchain == "eth" else bnb_usd_price
                            )  # Calculate value in USD
                            message = (
                                f"🚨 Incoming transaction detected on {wallet_address}"
                            )
                            send_telegram_notification(
                                message, value, usd_value, tx["hash"], blockchain
                            )
...
def add(update, context):
    ...
    message = (
        f"Added {wallet_address} to the list of watched {blockchain.upper()} wallets."
    )
    context.bot.send_message(chat_id=update.message.chat_id, text=message)
